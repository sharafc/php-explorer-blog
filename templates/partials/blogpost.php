<article>
    ...
    <?php if (isset($blogPost['blog_imagePath'])) : ?>
        <img src="<?= DOMAIN_SUB_STRUCTURE . "/" . $blogPost['blog_imagePath'] ?>" class="teaser-image <?= $blogPost['blog_imageAlignment'] ?>" alt="<?= $blogPost['blog_headline'] ?>" title="<?= $blogPost['blog_headline'] ?>">
    <?php endif ?>
    <p><?= nl2br($blogPost['blog_content']) ?></p>
    <?php if (!isset($blogpostId)) : ?>
        <p class="right"><a href="/home/showBlogpost/<?= $blogPost['blog_id'] ?>">Show detail</a></p>
    <?php endif ?>
</article>